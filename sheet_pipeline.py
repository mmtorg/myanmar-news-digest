...
PROMPT_SPECIAL_RULES = (
    ...
    "\n"
    "使い分けの目安：\n"
    "- 記事内で政党名を初めて説明する箇所では、可能であれば\n"
    "  「国軍系連邦団結発展党（USDP）」を優先する。\n"
    "- 2回目以降で略称のみが問題になっている箇所では\n"
    "  「国軍系USDP党」または「国軍系政党」を用いて簡略化してよい。\n"
    "- 文脈上、特定の政党名ではなく「国軍側の与党勢力」として\n"
    "  一般的にまとめても問題ない場合は、「国軍系政党」を用いてよい。\n"
    "\n"
    "【武装組織名の訳語ルール（BGF関連）】\n"
    ...
    "\n"
    "使い分けの目安：\n"
    "- 組織の初出や、国軍との関係を読者に説明する必要がある箇所では、\n"
    "  「国軍傘下のカレン州国境警備隊（BGF）」を優先する。\n"
    "- 2回目以降で、すでに文脈から国軍傘下組織であることが明らかな場合は、\n"
    "  「国軍傘下のカレン州国境警備隊」または「国軍系勢力BGF」に短縮してよい。\n"
    "- 細かい組織名より「国軍側の武装勢力」である点を強調したい文脈では、\n"
    "  「国軍系勢力」とだけ記述してよい。\n"
)
