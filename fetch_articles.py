...
def _run_bbc_title_exclude_test(url: str) -> None:
    """指定URLのページタイトルだけで、BBCの“タイトル除外”が発火するか最小テスト"""
    BBC_EXCLUDE_TITLE_KEYWORDS = [
    "နိုင်ငံတဝန်းသတင်းများ အနှစ်ချုပ်",
    ]
    BBC_EXCLUDE_TITLE_KEYWORDS = [
        unicodedata.normalize("NFC", s) for s in BBC_EXCLUDE_TITLE_KEYWORDS
    ]
    try:
        ...
        matched = [kw for kw in BBC_EXCLUDE_TITLE_KEYWORDS if kw in title_nfc]
        excluded = len(matched) > 0

        print("TITLE:", title_nfc)
        print("MATCHED_EXCLUDE_KEYWORDS:", matched)
        print("EXCLUDED_BY_TITLE =", excluded)  # True なら get_bbc_burmese_articles_for でも“スキップ”になる
    except Exception as e:
        print("TEST_ERROR:", e)
        sys.exit(1)

if __name__ == "__main__":
    TEST_BBC_URL = "https://www.bbc.com/burmese/articles/cly169p2pg7o"
    _run_bbc_title_exclude_test(TEST_BBC_URL)
    sys.exit(0)
