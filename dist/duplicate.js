...
2. 主要固有語の一致: 判定対象記事の主要固有語（特定の地名・人名・組織名・法律や制度の特定側面・具体的な行動内容）が、アーカイブ記事の見出しにもほぼ登場していること
   ※ 大テーマ（「燃料」「パスポート法」「投降者」など）が共通するだけでは不十分

◆ 補強条件（事象が一致する前提で以下が一致するほど確度が高い）
3. 同一地域: 同じ地名（州・管区・都市・郡区レベル）が共通して登場する
4. 同一主体: 同じ組織・人物（国軍・PDF・KNUなど）が主語・関与者として共通する
5. 同一時点: 同じ日付・時期（「3月12日」「先週」等）を指している
6. 同一数値: 死者数・被害額・人数などの具体的数値が共通して登場する
7. 同一発言: 同じ人物・組織による同じ声明・発言が示されている
...
- 同じ大テーマを扱うが、具体的な事象・側面が異なる記事
  例：「燃料価格高騰」と「QRコード給油制限による影響」は別トピック
  例：「パスポート法でのUID義務化（海外大使館での例外規定）」と「パスポート法の制定・施行」は別トピック
  例：「国軍が武装勢力500人投降と発表」と「PDF投降者への金銭支給」は別トピック
- 判定対象記事の特定地名・特定制度の特定側面・特定人物の具体行動がアーカイブ記事に含まれていない場合
...
- 迷う場合は「同一トピックではない」と判定すること（過剰マッチより見落としの方が許容される）
